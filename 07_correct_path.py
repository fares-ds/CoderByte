"""
Correct Path
Have the function CorrectPath(str) read the str parameter being passed, which will
represent the movements made in a 5x5 grid of cells starting from the top left position.
...
def correct_path(path):
    d_count, u_count, r_count, l_count, missing = 0, 0, 0, 0, 0
    for digit in path:
        if digit == 'd':
            d_count += 1
        elif digit == 'u':
    ...
    # return f"R: {r_count}, L:{l_count}, U:{u_count}, D:{d_count}, Missing:{missing}"


print(correct_path("???rrurdr?"))
